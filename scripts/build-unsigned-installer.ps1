...
try {
    & bun run tauri build --features desktop --config backend/tauri.conf.json --no-bundle --no-sign
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        ...
        & bun run tauri bundle --features desktop --config backend/tauri.conf.json --bundles nsis --no-sign
